...
def ingest_external_ildae():
    """일위대가DB/_공통/_외부원본/ 의 외부 표준일위대가 XLSX를 헤더 기반으로 통합.
...
1. 조달청/국토부에서 최신 표준일위대가 XLSX를 받아 `일위대가DB/_공통/_외부원본/` 하위에 둔다.
2. 정본을 `일위대가DB/_공통/표준일위대가_2026/` 로 복사한다(공종 폴더 또는 `국도하천항만/`).
