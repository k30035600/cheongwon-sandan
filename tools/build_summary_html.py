...
<div class="ok-box">
  <b>교정 완료({GEN_DATE}).</b> 과거 「행 번호 불일치 오패치」(살수↔세륜세차 단일 행 ≈1.36조)와 저신뢰 자동확정 이상치를
  내용 기반 매칭·보수적 재산출로 교정하여, 현재 직접공사비 <b>{eok(tot["tot"])}억({won(tot["tot"])}원)</b>으로 확정되었다.
  통합내역 = 공종별 합계 일치, 오패치 0건. 본 문서는 <code>총괄표.xlsx</code>에서 자동 생성된다(<code>tools/build_summary_html.py</code>).
</div>
...
<div class="summary">
  <ol>
    <li><b>직접공사비 합계</b> — <b>약 {eok(tot["tot"])}억원</b>({won(tot["tot"])}원). <span class="meta">※ 행 불일치 오패치·저신뢰 이상치 교정 완료. 03 전기(지구외)는 02와 동일 내역서(중복·#REF!)로 합계에서 제외.</span></li>
...
<div class="note">
  ...
  <ol>
    ...
    <li><b>저신뢰 자동확정 교정</b> — 미매칭 근접매칭(H) 중 내역품명↔DB매칭품명 불일치 29건을 보수적 재산출(가로지지대·경고테이프·우수받이·흄관하차비·점자블록). 상세는 <code>검토_품목검증.xlsx</code>·<code>검토_오매칭교정_품셈산출.xlsx</code>.</li>
